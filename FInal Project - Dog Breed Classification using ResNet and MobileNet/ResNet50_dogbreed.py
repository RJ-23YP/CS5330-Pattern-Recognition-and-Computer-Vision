...
class Net(torch.nn.Module):
    ...
    def __init__(self, base_model, base_out_features, num_classes):
        super(Net, self).__init__()  # Call the initializer of the superclass (torch.nn.Module)
...
def load_model(model_path):
    ...
    for param in resnet.parameters():
        param.requires_grad = False  # Set parameters to not update during training
    model = Net(resnet, resnet.fc.out_features, num_classes=120)  # Instantiate the Net class with the ResNet50 model
    model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))  # Load the trained model weights
    model.eval()  # Set the model to evaluation mode, useful for inference
    return model
...
def main(argv):
    MODEL_PATH = '/home/newusername/PRCV/final_project/MODEL/50model.pth'
